...
impl Png {
    ...
    pub fn chunk_by_type(&self, chunk_type: &str) -> Option<&Chunk> {
        self.chunks.iter().find(|chunk| {
            let chunk_type_string = chunk.chunk_type().to_string();
            chunk_type_string == chunk_type
        })
    }
...
impl TryFrom<&[u8]> for Png {
    ...
    fn try_from(bytes: &[u8]) -> Result<Png> {
        ...
        let (header, body) = bytes.split_at(Self::STANDARD_HEADER.len());
        ...
        let mut body = body;
        while !body.is_empty() {
            let chunk = Chunk::try_from(body)?;
            let chunk_length = chunk.length() as usize;
            let (_, rest) = body.split_at(chunk_length + Chunk::METADATA_BYTES);
            body = rest;
            chunks.push(chunk);
        }
